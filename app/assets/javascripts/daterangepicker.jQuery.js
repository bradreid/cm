...
jQuery.fn.daterangepicker = function(settings){
    new DateRangePickerClass(settings, jQuery(this));
};

function DateRangePickerClass(settings, input){
    var rangeInput = input;
    $(".datepickertheme." + input.attr('id')).remove();

    //defaults
    var options = jQuery.extend({
    	presetRanges: [
    	    // [br] this seems like a hack.  Is there a better way to clear the input field?
            {text: 'Clear', dateStart: function(){ return null;  }, dateEnd: function(){ return null;  }},
    ...
    var rpPresets = (function(){
      ...
      jQuery.each(options.presets, function(key, value) {
        var html = '<li class="ui-daterangepicker-'+ key +' preset_'+ x +' ui-helper-clearfix ui-corner-all"><span class="ui-icon ui-icon-triangle-1-e"></span><a href="#">'+ value +'</a></li>'
        jQuery(html)
          .appendTo(ul);
        x++;
      });
      ...
      var doneBtn = jQuery('<button class="btnDone ui-corner-all">'+ options.doneButtonText +'</button>')
        .click(function(){
      ...
      ul.find('li')
        ...
        .click(function(){
          ...
          jQuery(this).addClass('ui-state-active').clickActions(rp, rangeInput, doneBtn, rpPickers, options);
          return false;
        });
      return ul;
    })();
    ...
    //wrap and position
    if (null == options.posX){
      options.posX = rangeInput.position().left-20;
    }
    if (null == options.posY){
      options.posY = rangeInput.position().top + rangeInput.height() + 10
    }

    var wrapper = jQuery('<div class="datepickertheme '+ rangeInput.attr('id')+ '"></div>');
    wrapper.css('left', options.posX);
    wrapper.css('top', options.posY);
    jQuery(options.appendTo).append(wrapper);

    //inject rp
    // jQuery(options.appendTo).append(rp);
    $(".datepickertheme." + input.attr('id')).append(rp);
...
jQuery.fn.clickActions = function(rp, rangeInput, doneBtn, rpPickers, options){
